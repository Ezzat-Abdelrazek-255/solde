...
export function BudgetFilter({
  ...
  React.useEffect(() => {
    budgetsContext.setFilters((filters) =>
      filters.map((filter) => {
  ...
  React.useEffect(() => {
    if (budgetsContext.filters.every((filter) => filter.filters.length === 0)) {
      clearActiveFilters();
    }
  }, [
    budgetsContext.filters.reduce(
            ...
            <span
              className={`${
                activeFilters.length > 0
                  ? "border-r-2 border-brand-accent pr-2"
                  : ""
              }`}
            >
              {label}
